...
**Answer** I got rid of ", guess_max=195865" because every time I tried my entire computer shut down, but with that I got no errors.
...
**Answer**Amount is <dbl>, name is <chr>, and id is <dbl>
```{r}
...
**Answer**: 195,865 for NA, 0 for Subway since there is only one row
...
**Answer**: 135,073, which is the same as the number for forgiveness_amount and forgiveness_amount when run separately. NA represents loans that haven't been forgiven, possibly
```{r}
ppp_maryland_loans %>%
...
**Answer** $10 million is not suspiscious as that is the max loan amount, but $6 is definitely interesting because it is such a small amount of money. I call could clear this up.
```{r}
...
**Answer** The column headers are inconsistently styled, there’s a fully duplicated row, and the zip field mixes five digit ZIP codes and nine digit ZIP codes.
...
**Answer** "Problem with `summarise()` column `total_amount`. ℹ `total_amount = sum(amount)`. x invalid 'type' (character) of argument," which means total_amount is a <chr> data type
...
**Answer** 1 duplicate for Stuart Jones LLC
