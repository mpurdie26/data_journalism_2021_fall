...
#load data
ppp_per_state <- read_csv("data/state_ppp_all.csv")
...
top10_ppp_states_per_100k <- ppp_per_state %>%
  mutate(state = case_when(
    state == "S Dakota" ~ "South Dakota",
    state == "N Dakota" ~ "North Dakota",
    TRUE ~ state)) %>%
  inner_join(state_populations, by=c("state")) %>%
  select(-GEOID, -variable, -moe) %>%
  mutate(total_loans_per_100k = total_ppp_loans/population*100000) %>%
  arrange(desc(total_loans_per_100k)) %>%
  head(10)

top10_ppp_states_per_100k %>%
  ggplot() +
  geom_bar(aes(x=reorder(state, total_loans_per_100k), weight=total_loans_per_100k), fill="steelblue") +
  coord_flip() +
  labs(
...
wv_ppp_over_25k %>%    
  ...
  labs(
    title="Amount of PPP Loan Approved vs. Forgiven for WV Loans More Than $25,000",
    x = "Amount Approved",
    y = "Amount Forgiven",
    caption = "source: SBA PPP loan database"
  )
...
```
**A3.**  
There is a straight line going through the middle of the plot, representing loans in which the amount approved is the same as amount forgiven. Other points under the line are also represented in a line, possibly showing percent frames forgiven from SBA.
-30-
