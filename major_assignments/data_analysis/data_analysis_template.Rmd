...
police_shootings_by_race %>%
  full_join(national_grouped_shootings_w_acs) %>%
  mutate(per_capita_shootings_per_race = total_shot_per_race/total_pop_per_race*100000)
...
``` {r}
police_shootings_by_state_and_race <- new_shootings_w_acs  %>%
  mutate(GEOID = str_sub(census_tracts_geoid, start=1L, end=2L)) %>%
  group_by(state, race) %>%
  summarize(count = n(), GEOID) %>%
  distinct()
  
police_shootings_by_state_and_race %>%
  full_join(state_census_data)
police_shootings_by_state_and_race
